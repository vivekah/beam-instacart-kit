...
class NonprofitAPI {
    class func getNonprofits(at storeID: String,
        ...
        Network.shared.get(urlPath: "chains/nonprofits/?chain=\(chainID)&postal_code=\(storeID)&store=89&partner_user_id=\(userID)&show_community_impact=true&lan=\(language)",
    ...
    class func viewFavorite(_ completion: ((BK_INSFavoriteCopy?, BeamError) -> Void)? = nil) {
        ...
        let successHandler: (JSON?) -> Void = { favoriteJSON in
            BKLog.info("did get transaction info \(favoriteJSON)")
